...
class Smart:
    ...
    def process_smart(self, device_name:str) -> dict:
        ...
        match data['device']['type']:
            case 'nvme':
                self._process_nvme(data, logical_block_size)

            case 'ata' | 'sat' | 'sata':
                self._process_ata(data, logical_block_size)
    ...
    #
    def _process_nvme(self, data: dict, block_size:int):
        ...
        read)...
        """
        data['host_read_bytes'] = (data[nvme_key]['data_units_read'] * block_size * 1000)
        data['host_write_bytes'] = (data[nvme_key]['data_units_written'] * block_size * 1000)

    def _process_ata(self, data: dict, block_size:int):
        ...
        data['host_read_bytes'] = ret['read']
        data['host_write_bytes'] = ret['write']
